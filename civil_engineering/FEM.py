...
def plot_frame(nodes, elements, elem_props=None, loads=None, constraints=None,
               load_scale=0.2, q_scale=0.2, show_labels=True):
    """
    Plot a 2D frame structure with color-coded members, nodal loads,
    distributed loads, and boundary conditions.
    ...
    nodes : ndarray (n_nodes x 2)
        Nodal coordinates [x, y].
    elements : ndarray (n_elems x 2)
        Element connectivity (node1, node2), 0-based or 1-based.
    elem_props : list of dicts, optional
        Each dict may include 'q' for distributed load (kN/m, local -Y),
        'type' ('beam', 'column', or 'rafter'), etc.
    loads : ndarray or list, optional
        [[DOF_index, force_value], ...] — global DOF loads (3 DOF per node).
    constraints : list or ndarray, optional
        List of fixed DOFs (0-based). Example [0, 1, 2, 6, 7, 8].
    load_scale : float
        Arrow length scale for nodal loads.
    q_scale : float
        Arrow length scale for distributed loads.
    show_labels : bool
        If True, labels nodes and elements.
    """
    ...
    for i, (x, y) in enumerate(nodes):
        ...
        plt.plot(x, y, "o", color=color, markersize=8)
        if show_labels:
            plt.text(x + 0.1, y + 0.05, f"N{i+1}", fontsize=10)
    # --- Plot elements ---
    beam_count = column_count = rafter_count = 0
    for i, (n1, n2) in enumerate(elements):
        x = [nodes[n1, 0], nodes[n2, 0]]
        y = [nodes[n1, 1], nodes[n2, 1]]
        dx, dy = x[1] - x[0], y[1] - y[0]

        if abs(dy) < 1e-6:  # horizontal → beam
            color = "g"
            beam_count += 1
            label = f"B{beam_count}"
        elif abs(dx) < 1e-6:  # vertical → column
            color = "y"
            column_count += 1
            label = f"C{column_count}"
        else:  # sloped → rafter
            color = "b"
            rafter_count += 1
            label = f"R{rafter_count}"
        plt.plot(x, y, color=color, linewidth=2)
        if show_labels:
            xm, ym = np.mean(x), np.mean(y)
            plt.text(xm, ym, label, fontsize=10, color="k",
        ...
        if elem_props is not None and i < len(elem_props):
            q = elem_props[i].get("q", 0)
            if abs(q) > 1e-9:
                L = np.hypot(dx, dy)
                nx = dx / L
                ny = dy / L
                perp_x, perp_y = -ny, nx  # local -Y dir
    ...
    # --- Nodal loads (DOF-based) ---
    if loads is not None and len(loads) > 0:
        node_loads = np.zeros((n_nodes, 2))
        for dof, val in loads:
            node = int(dof // 3)
            dirn = int(dof % 3)
            if node < n_nodes and dirn < 2:  # ignore rotations
                node_loads[node, dirn] += val
        for i, (Fx, Fy) in enumerate(node_loads):
            if abs(Fx) > 1e-6 or abs(Fy) > 1e-6:
                ...
                plt.arrow(x, y, load_scale * Fx, load_scale * Fy,
                          head_width=0.15, head_length=0.25,
                          fc='red', ec='red', linewidth=1.5, zorder=6)
    # --- Legend ---
    legend_items = [
        plt.Line2D([], [], color='g', lw=2, label='Beams (B1, B2, ...)'),
        plt.Line2D([], [], color='y', lw=2, label='Columns (C1, C2, ...)'),
        plt.Line2D([], [], color='b', lw=2, label='Rafters (R1, R2, ...)'),
        plt.Line2D([], [], color='purple', marker='o', linestyle='None', label='Fixed support'),
        plt.Line2D([], [], color='green', marker='o', linestyle='None', label='Fixed X'),
        plt.Line2D([], [], color='brown', marker='o', linestyle='None', label='Fixed Y'),
        plt.Line2D([], [], color='black', marker='o', linestyle='None', label='Free node'),
        plt.Line2D([], [], color='red', marker=r'$\rightarrow$', linestyle='None', label='Load (nodal/distributed)')
    ]
    plt.legend(handles=legend_items, loc='upper right', fontsize=8)
    plt.show()
...
def FEM2D_frame2(nodes, elements, elem_props, loads=None, constraints=None, default_E=210e6):
    """
    2D Frame FEM solver (axial + bending).
    All quantities are in the kN–m system:
        - Forces in kN
        - Moments in kN·m
        - Lengths in m
        - E in kN/m²
    ...
    for e in range(n_elems):
        ...
        L = np.hypot(dx, dy)
        C, S = dx / L, dy / L
        prop = elem_props[e] if e < len(elem_props) else {}
        A = float(prop.get('A', 1e-6))
        I = float(prop.get('I', 0.0))
        E = float(prop.get('E', default_E))
        q = float(prop.get('q', 0.0))
        etype = prop.get('type', 'beam')
        # Axial stiffness
        k_ax = E * A / L
        ...
        k_local = np.array([
            [  k_ax,     0,        0,   -k_ax,     0,        0],
            [  0,     k_b11,   k_b12,     0,   -k_b11,   k_b12],
            [  0,     k_b12,   k_b22,     0,   -k_b12, k_b22_off],
            [-k_ax,     0,        0,    k_ax,     0,        0],
            [  0,    -k_b11,  -k_b12,     0,    k_b11,  -k_b12],
            [  0,     k_b12, k_b22_off,   0,   -k_b12,   k_b22]
        ])
        # Transform to global
        T = T_matrix(C, S)
        k_global = T.T @ k_local @ T
        dof = [3*n1, 3*n1+1, 3*n1+2, 3*n2, 3*n2+1, 3*n2+2]
        K[np.ix_(dof, dof)] += k_global
        # Equivalent nodal load from uniform q (local -Y)
        if abs(q) > 0:
            f_local = np.array([
                0, q*L/2,  q*L**2/12,
                0, q*L/2, -q*L**2/12
            ])
            F[np.array(dof)] += T.T @ f_local
    # Apply nodal loads
    for dof_idx, val in (loads or []):
        F[int(dof_idx)] += float(val)
    ...
    shear_forces = np.zeros((n_elems, 2))
    end_moments = np.zeros((n_elems, 2))
    u_local_all = np.zeros((n_elems, 6))
    internal_local = np.zeros((n_elems, 6))
    for e in range(n_elems):
        n1, n2 = elements[e]
        x1, y1 = nodes[n1]
        x2, y2 = nodes[n2]
        ...
        if abs(q) > 0:
            ...
            f_local = np.zeros(6)
        internal = k_local @ u_e - f_local
        internal_local[e] = internal
        axial_forces[e] = (E*A/L)*(u_e[3]-u_e[0])
        shear_forces[e] = [internal[1], internal[4]]
        end_moments[e] = [internal[2], internal[5]]
    # Pack results
    return {
        "u_global": u,
        "reactions": reactions,
